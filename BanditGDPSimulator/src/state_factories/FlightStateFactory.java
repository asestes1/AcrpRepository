...
public final class FlightStateFactory {
	...
	public static FlightState makeFullCapacityState(DateTime startTime, Duration duration, int capacity) {
		return makeFullCapacityState(startTime, startTime.plus(duration), capacity);
	}

	public static FlightState makeFullCapacityState(DateTime startTime, DateTime endTime, int capacity) {
		ConstantRunwayDistribution runwayTime = new ConstantRunwayDistribution();
		DateTime nextArrival = startTime;
		SortedSet<Flight> arrivals = new TreeSet<Flight>(new FlightDateTimeFieldComparator(Flight.aETAFieldID));

		SortedSet<Flight> departures = new TreeSet<Flight>(new FlightDateTimeFieldComparator(Flight.aETDFieldID));
		// For the first hour, create flights which are in the air and which are
		// separated
		...
		while (nextArrival.isBefore(endTime)) {
			...
			if (nextArrival.minusHours(flightTime).isBefore(startTime)) {
				Flight nextFlight = new Flight("0", 200, 100, nextArrival.minusHours(flightTime), nextArrival, null,
						null, nextArrival.minusHours(flightTime), nextArrival, true, false, false, false, Duration.ZERO,
						Duration.ZERO, Duration.ZERO, Duration.ZERO, Duration.ZERO);
				arrivals.add(nextFlight);

			} else {
				Flight nextFlight = new Flight("0", 200, 100, nextArrival.minusHours(flightTime), nextArrival, null,
						null, nextArrival.minusHours(flightTime), null, true, false, false, false, Duration.ZERO,
						Duration.ZERO, Duration.ZERO, Duration.ZERO, Duration.ZERO);
		...
		FlightState myFlights = new FlightState(new HashSet<Flight>(), arrivals, departures, new HashSet<Flight>());
		return myFlights;
	}

	/**
	 * This is an implementation of FlightInitializer This assumes that all
	 * flights that were scheduled to depart before the current time left on
	 * time. Flights which haven't departed yet are given departure delays
	 *
	 */
	public static FlightState parseFullCapacityState(File input, DateTime startTime) throws Exception {
		Scanner myScanner = new Scanner(input);
		return parseFullCapacityState(myScanner, startTime);
	}

	public static FlightState parseFullCapacityState(Scanner scanner, DateTime startTime) throws Exception {
		String input = "";
	...
	public static FlightState parseFlightState(File file, Interval runInterval, DateTimeZone timeZone, int formatId)
			throws Exception {
		return parseFlightState(file, runInterval, timeZone, formatId, new HashMap<Integer, Distribution<Integer>>(),
	...
	public static FlightState parseFlightState(File file, Interval runInterval, DateTimeZone timeZone, int formatId,
		...
		return parseFlightState(file, runInterval, timeZone, formatId, intFieldGenerators,
				new HashMap<Integer, Distribution<DateTime>>(), new HashMap<Integer, Distribution<Duration>>());
	}
	...
	public static FlightState parseFlightState(File file, Interval runInterval, DateTimeZone timeZone, int formatId,
		...
		if (formatId == FlightFactory.BTS_FORMAT_ID || 
				formatId == FlightFactory.ADL_FORMAT_ID) {
		...
		SortedSet<Flight> airborneFlights = new TreeSet<Flight>(new FlightDateTimeFieldComparator(Flight.aETAFieldID));
		SortedSet<Flight> sittingFlights = new TreeSet<Flight>(new FlightDateTimeFieldComparator(Flight.aETDFieldID));

		DateTime startTime = runInterval.getStart();
		for (CSVRecord record : parser) {
			try{
				// Read the string and make a flight
				Flight nextFlight = FlightFactory.parseFlight(record, startTime, timeZone, formatId);			
				...
				if (nextFlight.getOrigETD().isBefore(nextFlight.getOrigETA())
						&& runInterval.contains(nextFlight.getOrigETA())) {
	...
	public static FlightState delaySittingFlights(FlightHandler myFlightHandler, FlightState flightState) {
		SortedSet<Flight> myNewSittingFlights = new TreeSet<Flight>(
	...
	public static FlightState parseBtsFlightFile(String btsDirName, String filePrefix, LocalDate date,
			Interval runInterval, DateTimeZone timeZone, int formatId) throws Exception {
		String fileName = btsDirName + "/" + date.getYear() + "/" + date.getMonthOfYear() + "/";
		fileName += filePrefix + "_" + date.getYear() + "_" + date.getMonthOfYear() + "_" + date.getDayOfMonth()
				+ ".csv";
		File myFile = new File(fileName);
		return FlightStateFactory.parseFlightState(myFile, runInterval, timeZone, formatId);
	}

	public static FlightState parseAdlFlightFile(String adlDirName, String filePrefix, LocalDate date,
			Interval runInterval, DateTimeZone timeZone, int formatId) throws Exception {
		...
		String fileName = adlDirName + "/" + filePrefix + date.getYear() + monthString + dayString + ".csv";
		File myFile = new File(fileName);
		return FlightStateFactory.parseFlightState(myFile, runInterval, timeZone, formatId);
	}
